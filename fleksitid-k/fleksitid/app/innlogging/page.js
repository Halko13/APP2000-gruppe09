...
const InnloggingSide = () => {
    ...
    useEffect(() => {
        const hentBrukere = async () => {
            ...
            const brukerListe = brukerData.docs.map(dok => ({ id: dok.id, ...dok.data() }));
    ...
    const validerLogin = () => {
        ...
        if (valgtBruker) {
            if (valgtBruker.ErAdmin && valgtBruker.Passord === pin) {
                setLoginStatus('Logget inn som admin. Velkommen ' + valgtBruker.Fornavn + '!');
                // Til admin siden 
                window.location.href = '/admin';
            } else if (!valgtBruker.ErAdmin && valgtBruker.Passord === pin) {
                setLoginStatus(`Logget inn. Velkommen ${valgtBruker.Fornavn} ${valgtBruker.Etternavn}!`);
                // Til ansatt siden
            ...
            {aktivSide === 'VelgBrukerListe' ? (
                ...
                <div>
                    <Typography variant="h2" gutterBottom>
                        Velkommen {
                            erAdmin 
                            ? adminBrukere.find(admin => admin.brukernavn === valgtBrukerId)?.brukernavn 
                    ...
                    <Typography variant="h5" gutterBottom >
                        Logget på som: {
                            erAdmin 
                            ? adminBrukere.find(admin => admin.brukernavn === valgtBrukerId)?.brukernavn || 'Ukjent Admin'
                ...
                <Button variant="contained"  color="primary" fullWidth
                    ...
                    style={{ marginTop: 75}}
                    >Gå tilbake til hovedsiden</Button>
